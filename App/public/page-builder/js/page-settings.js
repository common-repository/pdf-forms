...
(function( $ ) {
	...
	window.PageBuilder = {
		...
		saveEvent: function( selectors ) {
            ...
            var onRadioInput = jQuery(result).find('input[id="pdfforms-main-settings[token-exist]-true"]');
            if (jQuery(onRadioInput[0]).attr('checked') === 'checked') {
              jQuery('input[id="pdfforms-main-settings[token-exist]-false"]').removeAttr('checked');
              jQuery('input[id="pdfforms-main-settings[token-exist]-true"]').attr('checked', 'checked');
            } else {
              jQuery('input[id="pdfforms-main-settings[token-exist]-true"]').removeAttr('checked');
              jQuery('input[id="pdfforms-main-settings[token-exist]-false"]').attr('checked', 'checked');
            }
